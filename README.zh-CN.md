...
> 前提：一台能访问 `gemini.google.com` 的 Linux 服务器（或本机）。

**Step 1. 拿到 cookie（最关键的一步）**

浏览器装一个 **Cookie-Editor** 扩展（Chrome / Edge / Firefox 应用商店都有），然后：

1. 登录 https://gemini.google.com
2. 点 Cookie-Editor 图标 → **Export as JSON**（不要用 F12 的 `document.cookie`，它读不到 `HttpOnly` 的 cookie）
3. 从导出的 JSON 里找到这两个值：
   - `__Secure-1PSID`（`g.a000...` 开头）
   - `__Secure-1PSIDTS`（`AKEyXz...` 或 `sidts-...` 开头）

拼成一行：`__Secure-1PSID=g.a000...; __Secure-1PSIDTS=AKEyXz...`
...
cat > /root/gemini-panel/accounts.json <<'EOF'
{
  "A": {
    "name": "my account",
    "cookies": "__Secure-1PSID=把Step1拿到的值填这里; __Secure-1PSIDTS=把Step1拿到的值填这里",
    "auth_user": ""
  }
}
EOF
...
看到 `* Running on http://0.0.0.0:4983` 就成功了。
...
把 `http://你的服务器IP:4983` 填进 RikkaHub / 任意 OpenAI 客户端（详见下方「客户端接入」）。

> 生产环境建议用 systemd + Caddy 反代（HTTPS），模板见 `deploy/`。
